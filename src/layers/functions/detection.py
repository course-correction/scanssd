...
class Detect(Function):
    ...
    @staticmethod
    def forward(cfg, num_classes, top_k, conf_thresh, gpus, loc_data, conf_data, prior_data):
        """
        Args:
        ...
        conf_thresh = min(conf_thresh)

        num = loc_data.size(0)  # batch size
        num_priors = prior_data.size(0)
        output = torch.zeros(num, num_classes,
                             top_k, 5, device=gpus)
        ...
        for i in range(num):
            ...
            for cl in range(1, num_classes):
                ...
                topk_scores = scores[sort_scores][:top_k]
                topk_boxes = boxes[sort_scores][:top_k]
