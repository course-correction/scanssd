...
class GTDBAnnotationTransform(object):
    ...
    def __call__(self, target, width, height):
        ...
        for box in target:
            res.append(
                [box[0] / width, box[1] / height, box[2] / width, box[3] / height, 0]
            )
...
class GTDBDetection(data.Dataset):
    ...
    def __init__(
        ...
        self.ids = [
            (self.root, "/".join(image_file.split("/")[-2:]).replace(".png", ""))
    ...
    def generate_metadata(self):
        ...
        for id in self.ids:
            ...
            if (self.split == "train" or self.split == "validate") and self.is_math[
                id[1]
            ]:
                ...
                for i in h:
                    for j in v:
                        ...
                        current_page_boxes = copy.deepcopy(
                            self.math_ground_truth[id[1]]
                        )
                        ...
                        for box in current_page_boxes:
                            if box_utils.intersects(image_box, box):
                                ...
                                if (
                                    feature_extractor.width(box) > 0
                                    and feature_extractor.height(box) > 0
                                ):
    ...
    def read_gt_annotations(self):
        ...
        for id in self.ids:
            if osp.exists(self._annopath % id):
                ...
                self.math_ground_truth[id[1]] = np.array([-1, -1, -1, -1]).reshape(
                    1, -1
                )
